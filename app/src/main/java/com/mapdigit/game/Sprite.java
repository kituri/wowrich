...
public class Sprite extends Layer {
	...
	public final boolean collidesWith(Bitmap image, int iX, int iY,
			boolean pixelLevel) {
		if (image == null)
			throw new IllegalArgumentException();
	...
	public void setImage(Bitmap img, int frameWidth, int frameHeight) {
		synchronized (this) {
			...
			if (frameWidth != getWidth() || frameHeight != getHeight()) {
				...
				// if necessary change position to keep the reference pixel in
				// place
	...
	public final void paint(Canvas g) {
		...
		int fx = w * (f % cols);
		int fy = h * (f / cols);
		Matrix matrix=createTransformationMatrix(transform);

		Rect srcRect = new Rect(fx, fy, fx+w , fy+ h);
		Rect dstRect = new Rect(getX(), getY(), getX()+w, getY()+h);
		Bitmap transImage=Bitmap.createBitmap(img, 0, 0, img.getWidth(), img.getHeight(), matrix, true);
		
		g.drawBitmap(transImage, srcRect, dstRect, null);
	}
	...
	private synchronized boolean collidesWithPixelLevel(Object o, int oX, int oY) {
		...
		while (another) {
			...
			switch (t.transform) {
			case TRANS_NONE:
				getRGB(t.img,tRgbData, 0, rW, fX + rX - t.getX(), fY + rY
						- t.getY(), rW, rH);
				sOffset = 0;
				sColIncr = 1;
				sRowIncr = 0;
				break;
			case TRANS_ROT180:
				getRGB(t.img,tRgbData, 0, rW, fX + fW - (rX - t.getX()) - rW
			...
			case TRANS_MIRROR:
				...
				break;
			case TRANS_MIRROR_ROT180:
				getRGB(t.img,tRgbData, 0, rW, fX + rX - t.getX(), fY + fH
				...
				break;
			case TRANS_ROT90:
				getRGB(t.img,tRgbData, 0, rH, fX + rY - t.getY(), fY + fH
						- (rX - t.getX()) - rW, rH, rW);
				sOffset = (rW - 1) * rH;
				sColIncr = -rH;
				sRowIncr = (rH * rW) + 1;
				break;
			case TRANS_MIRROR_ROT90:
				getRGB(t.img,tRgbData, 0, rH, fX + fW - (rY - t.getY()) - rH,
				...
				break;
			case TRANS_MIRROR_ROT270:
				getRGB(t.img,tRgbData, 0, rH, fX + rY - t.getY(), fY + rX
						- t.getX(), rH, rW);
				sOffset = 0;
				sColIncr = rH;
				sRowIncr = -(rH * rW) + 1;
			...
			if (o != t) {
				...
				if (o instanceof Sprite) {
					...
					for (int row = minR; row <= maxR; row++) {
						for (int col = minC; col <= maxC; col++) {
							...
							if (cell == 0) {
								...
								getRGB(img,rgbDataAux, c, rW, xSrc + minX, ySrc
					...
					getRGB(img,rgbDataAux, 0, rW, rX - oX, rY - oY, rW, rH);
					oOffset = 0;
					oColIncr = 1;
					oRowIncr = 0;
				}
	...
	static void getRGB(Bitmap img, int[] rgbData, int offset, int scanlength,
			int x, int y, int width, int height) {

		IntBuffer buffer = IntBuffer.allocate(img.getWidth() * img.getHeight());
		img.copyPixelsToBuffer(buffer);
		int[] rgb = buffer.array();
		int[] tempRGB = rgb;
		int dstWidth = Math.min(width, img.getWidth() - x);
		int dstHeight = Math.min(height, img.getHeight() - y);
		if (dstWidth != width || dstHeight != height) {
			tempRGB = new int[dstWidth * dstHeight];
			for (int i = 0; i < dstHeight; i++) {
				System.arraycopy(rgb, (i + y) * width + x, tempRGB, i
						* dstWidth, dstWidth);
			}
		}
		rgbData = tempRGB;
