...
//熔火之心
public class eUS_MC implements CustomRequest {
	...
	private final int Type = Constants.TYPE_US_MC;
	private final int BattleLV = 8;
	// 20% 30% 40% 16 14 12
//	private final int Difficulty = 12;
	...
	private String[] BossMessage = { 
			"【BOSS】玛格曼达：\n…………………………\n", 
			"【BOSS】埃克索图斯：\n尘归尘，土归土。",
			"【BOSS】拉格纳罗斯：\n现在轮到你们了，你们愚蠢的追寻拉格纳罗斯的力量，现在你们即将亲眼见到它。" };

	private int[] BossDifficulty = { 20, 23, 26 };
	//上下随机变动难度值
	private int BossDifficultyMix = -5;
	private int BossDifficultyMax =  5;
	//battleBonusEven 需要改动(掉物品)

	private final String raidMessage = "位于东部王国黑石山内的黑石深渊深处。" +
			"这里是受黑石矮人召唤的元素领主炎魔拉格纳罗斯的巢穴。";

	//
	public eUS_MC(Context _context, EvenOverRequest evenOverRequest) {
			...
			if(eBattleEven.RandomPer(80)){
				...
				Constants.ItemMessage = "战胜BOSS后，发现了一个宝箱。" + Constants.ItemMessage;
					...
					if(eBattleEven.RandomPer(50)){
						Constants.player[Constants.playerIndex].addEquips(1);
						Constants.ItemMessage = "打开宝箱，获得了 【" + Constants.player[Constants.playerIndex].getEquipsByID(1)
						.getName() + "】 。";
					}else{
						Constants.player[Constants.playerIndex].addEquips(2);
						Constants.ItemMessage = "打开宝箱，获得了 【" + Constants.player[Constants.playerIndex].getEquipsByID(2)
						.getName() + "】 。";
					}
				}
